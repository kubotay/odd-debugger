 ...
 */
public class ODDMethodExitEventListener implements OddMethodEventListener {

    private MethodExitRequest request;
    private JDIThread jdiThread;

    public ODDMethodExitEventListener(MethodExitRequest request, JDIThread jdiThread) {
        this.request = request;
        this.jdiThread = jdiThread;
    }

    public ODDMethodExitEventListener(MethodExitRequest request) {
        this.request = request;
    }
    public void removeFromObservable(){
        jdiThread.removeJDIEventListener(this, this.request);
        jdiThread.getEventRequestManager().deleteEventRequest(this.request);
    }
    public MethodExitRequest getRequest(){
        return this.request;
    }
    @Override
    public boolean handleEvent(Event event, JDIDebugTarget jdiDebugTarget, boolean b, EventSet eventSet) {
        ...
        try {
            ...
            if(objectReference != null){
                System.out.println("OMExEL:("+ objectReference.referenceType().name() + " " + objectReference.uniqueID() + ") " + ((MethodExitEvent) event).method());
                String s = "MX#";
                ...
                System.out.println("OMExEL:(class)" + e.method().declaringType().name());
            }
