...
public class Token {
    ...
    public long drawToken(float[] p, int num){
    	tokenId = num;
        integrator[0].target(p[0]);
        integrator[1].target(p[1]);
        integrator[0].tUpdate();
        integrator[1].tUpdate();
    ...
    public boolean checkTokenClick(float mouseX, float mouseY){
        ...
        float d = javanimation.dist(integrator[0].getValue(), integrator[1].getValue(), mouseX, mouseY);
        if(d < r){
            System.out.println("トークン：" + this.threadId);
