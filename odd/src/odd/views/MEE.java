...
public class MEE implements Event{
    ...
    public MEE(String oddMethod, List<Instance> listI, ArrayList<Event> listM, ArrayList<Token> listT, javanimation p){
    	papplet = p;
    	this.instanceList = listI;
        this.methodList = listM;
        this.tokenList = listT;

        String[] methodData = oddMethod.split("#");
        this.id = Long.parseLong(methodData[0]);
        this.methodName = methodData[1];
        this.returnType = methodData[2];
        this.modifiers = Integer.parseInt(methodData[3]);
        int index = 5;
        if(methodData[4].equals("ARGS")){
        	int argLen = Integer.parseInt(methodData[index]);
        	index++;
        	for(int i = 0; i < argLen; i++){
        		args.add(new Tuple<String,String,String>(methodData[index],methodData[index+1], methodData[index+2]));
        		index+=3;
        	}
        }
        if(this.methodName.equals("<init>")){
        ...
        this.threadName = methodData[index];
        this.threadId = Long.parseLong(methodData[index+1]);
        this.isStatic = this.isStatic();
    }
