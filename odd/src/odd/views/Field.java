...
public class Field {
    ...
    public Field(String[] objectData, int index, javanimation p){ //ODD用フィールドコンストラクタ
    	papplet = p;
    	//this.type = objectData[index];
    	this.name = objectData[index+1];
    	this.value = objectData[index+2];
    	papplet.textSize(12);
    	this.textLength = papplet.textWidth(this.access + this.type + this.name+"=" + this.getValue());
        papplet.textSize(javanimation.getFontSize()*papplet.getMagnification()/100);
    }
