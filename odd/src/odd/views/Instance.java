...
public class Instance {
    ...
    private boolean isRoot = true; // ツリー構造のルートオブジェクトならtrue
    ...
    public Instance(String[] objectData, javanimation p){
    	papplet = p;
    	this.id = Long.parseLong(objectData[0]);
    	this.className = objectData[1];
    	this.animation_flag = 1;
    	integrator = new Integrator[5];
        integrator[0] = new Integrator(this.point[0]);
        integrator[1] = new Integrator(this.point[1]);
        ...
        int index = 4;
        for(int i = 0; i < Integer.parseInt(objectData[3]); i++){
        	Field fi = new Field(objectData,index, papplet);
            this.fieldList.add(fi);
            index += 3;
            fieldCount++;
            if(fi.getTextLength() >= this.maxLength){
    ...
    public void presetSList(List<Instance> instanceList){
    	String list = "[";
        ...
        if(list.startsWith("[<")){
            this.removeSList();
            this.setSList(instanceList, list);
        }
    }

    public float[] drawBox(float[] maxWH, float num){
        if(this.drawFlag && (MainPanel.getAnimationFlag() || this.checkArea())) {
            float[] tmp = {0, 0};
            int count =0;
            ...
            if(this.checkArea()){
	            ...
	            if(papplet.textWidth(str) <= movedPoint(5)+1) papplet.text(str, movedPoint(1), movedPoint(2) + papplet.textDescent());
	                ...
	                for (int i = 0; i < this.fieldCount; i++) {
	                    ...
	                    if (fi.getIsMethod()) {
	                        switch(fi.getTextColor()) {
	                            case 0: papplet.fill(0xE6,0x00,0x12, fi.getTextAlpha()); break;
	                            case 1: papplet.fill(0x1D,0x20,0x88, fi.getTextAlpha()); break;
	                            case 2: papplet.fill(0x00,0x99,0x44, fi.getTextAlpha()); break;
	                            case 3: papplet.fill(0xF3,0x98,0x00, fi.getTextAlpha()); break;
	                            case 4: papplet.fill(0x00,0x9E,0x96, fi.getTextAlpha()); break;
	                            case 5: papplet.fill(0x92,0x07,0x83, fi.getTextAlpha()); break;
	                            case 6: papplet.fill(0xE5,0x00,0x4F, fi.getTextAlpha()); break;
	                            case 7: papplet.fill(0x00,0x68,0xB7, fi.getTextAlpha()); break;
	                            case 8: papplet.fill(0x8F,0xC3,0x1F, fi.getTextAlpha()); break;
	                        }
    ...
    public void accessWatchpoint(AWE awe){
    ...
    public String getFullClassName(){return this.className;}
        ...
        for(Field field : fieldList){
            for(int i = 0; i < mxe.getNumFields(); i++){
                if(mxe.getFieldName(i).equals(field.getName())){
                    if(!(mxe.getFieldValue(i).equals(field.getValue()))){
            ...
            if ((mouseX >= movedPoint(1)-movedPoint(5)/2) && (mouseX <= movedPoint(1)+movedPoint(5)/2)) {
                if((mouseY >= movedPoint(2)-movedPoint(6)/2) && (mouseY <= movedPoint(4)+movedPoint(6)/2+movedPoint(7))) {
                    ...
                    if(y <= 0){ // id部分をクリック
                    	...
                    	int num = (int)(y / (movedPoint(7)/this.fieldCount));
                    	if(num < this.fieldCount){
                    		String s;
                    		s = this.fieldList.get(num).checkFieldClick();
                    		if(papplet.getWateState() == 4){
                    			papplet.setFieldName(s + ":" + this.id);
                    		}
                    	}
                    }
                    return true;
